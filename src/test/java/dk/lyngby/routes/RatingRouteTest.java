...
class RatingRouteTest {
    ...
    @Test
    void getRatingsByPictureId() {
        given()
                .contentType("application/json")
                .header("Authorization", "Bearer " + user_1Token)
                .when()
                .get(BASE_URL + "/ratings/A red building with red balconies and a clock")
                .then()
                .statusCode(200);
    }

    @Test
    void deleteAllRatingsFromPicture() {
        given()
                .contentType("application/json")
                .header("Authorization", "Bearer " + adminToken)
                .when()
                .delete(BASE_URL + "/ratings/A red building with red balconies and a clock")
                .then()
                .statusCode(204);
    }

    @Test
    void deleteRatingsFromUserPictures() {
        given()
                .contentType("application/json")
                .header("Authorization", "Bearer " + adminToken)
                .when()
                .delete(BASE_URL + "/ratings/" + admin.getUsername())
                .then()
                .statusCode(204);
    }

    @Test
    void deleteRatingFromPicture() {
        given()
                .contentType("application/json")
                .header("Authorization", "Bearer " + adminToken)
                .when()
                .delete(BASE_URL + "/ratings/rating/A wooden table topped with a vase filled with flowers/" + admin.getUsername())
                .then()
                .statusCode(204);
    }
